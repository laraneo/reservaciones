    ...
    */
    ...
    public function index(Request $request)
    {
        if($request['category']  === null) {
            if(Session::get('CustomerDrawsControllerIndexQueryStrings')) {
                $selectedCategory = Session::get('CustomerDrawsControllerIndexQueryStrings');
                $category = $selectedCategory ? "AND p.category_id = ".$selectedCategory." " : '';
            } else {
                $category = $request['category'] ? "AND p.category_id = ".$request['category']." " : '';
                $selectedCategory = $request['category'] !== null ? $request['category'] : null;
                Session::put('CustomerDrawsControllerIndexQueryStrings',$selectedCategory);
            }
        } else {
            $category = $request['category'] ? "AND p.category_id = ".$request['category']." " : '';
            $selectedCategory = $request['category'] !== null ? $request['category'] : null;
        ...
        return view('customer.draws.index', compact('draws','categories','selectedCategory'));
    }
}
