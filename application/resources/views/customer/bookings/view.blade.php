		...
		if (PlayerSlot == 1) {
			...
			if (player1.trim().toLowerCase() == 'favorites') {
				...
				//		if (player1)
				{
					...
					package_id ') }}' +
						"&categoryType=" + '{{ Session::get('
					categoryType ') }}' +
						"&packageType=" + '{{ Session::get('
					packageType ') }}';
		...
		ajaxRequest.onreadystatechange = function() {
			...
			if (ajaxRequest.readyState == 4) {
				...
				*/
				...
				//ajaxDisplay.value = ajaxRequest.responseText; 
				//ajaxDisplay.innerHTML = ajaxRequest.responseText; 
				window.location.href = `${url}/customer/booking/{{ $booking->id }}`;
			}
				...
				<div class="panel-body">
					<div class="account_details_view">
						...
						<div class="row table-row booking-players player-{{$player->id}} active">
							...
							<div class="col-md-2 bold-font {{ $player->isConfirmed()  ? 'label-success' : 'label-danger' }}">
								<p align="center" style="color:white; margin: 0 auto; padding: 5px"><strong> {{ $player->PlayerConfirmedStatus() }}</strong></p>
							</div>
					...
					<form name='myForm' class="search-helper-player hidde-form" method="post" action="outputHelper.php" style="margin-top: 20px">
						...
						<div class="row">
							...
							<div class="col-md-3">
								<input type="text" rows="" class="form-control has-success has-feedback form-control-lg" name="player1" id="player1" placeholder="{{ __('app.player_placeholder') }}" autocomplete="off">
							</div>
							<div id='ajaxDivPlayer1'>
							</div>
							<div class="col-md-6">
								<input type="text" class="form-control form-control-lg" name="player1Name" id="player1Name" autocomplete="off" disabled>
							</div>
						</div>
						...
						<div class="row">
							...
							<div class="col-md-9">
								...
								<input type="hidden" class="form-control form-control-lg" name="player1Type" id="player1Type" autocomplete="off">
								<input type="hidden" class="form-control form-control-lg" name="player1Status" id="player1Status" autocomplete="off">
