...
if ($command == "favorites") // query favorites
{
	...
	
	{
		...
		$resultFavorites = mysqli_query($connection, $queryFavorites); 
		  
		if ($resultFavorites) 
		{ 
			$rowGroupFavorites = mysqli_num_rows($resultFavorites); 
		   // printf("Number of row in the table : " . $row); 
			if ($rowGroupFavorites>0) 
			{
				while($row = mysqli_fetch_array($resultFavorites)){
		...
		@mysqli_free_result($resultFavorites); 	
	}
...
else if ($command == "partners") // query frequent participants
{
	...
	
	{
		...
		$queryPartners =  "SELECT p.doc_id, COUNT(p.doc_id),    p.player_type, 
					CASE 
						WHEN p.player_type=0 THEN (SELECT CONCAT( p1.first_name,' ', p1.last_name) FROM users p1 WHERE p1.doc_id=p.doc_id  )
						WHEN p.player_type=1 THEN (SELECT CONCAT( p2.first_name,' ', p2.last_name) FROM guests p2 WHERE p2.doc_id=p.doc_id  )
					END AS PlayerName	
		...
		$resultPartners = mysqli_query($connection, $queryPartners); 
		  
		if ($resultPartners) 
		{ 
			$rowGroupPartners = mysqli_num_rows($resultPartners); 
		   // printf("Number of row in the table : " . $row); 
			if ($rowGroupPartners>0) 
			{
				while($row = mysqli_fetch_array($resultPartners)){
					//$playername = $row['first_name'] . " " . $row['last_name'];
					$playername = $row['PlayerName'];
					$playerid = $row['doc_id'];
					$member =   $playerid . $fieldinternal_separator .  $playername ;
		...
		@mysqli_free_result($resultPartners); 	
	}
...
else if ($command == "include") // include player
{
   ...
   $queryGroup = "select * from users where doc_id='" . $doc_id . "'";
	...
	{ // Chequea que no se exceda el máximo de partidas por día
		...
		$resultBookingPlayerCount = mysqli_query($connection, $queryBookingPlayerCount); 
		  
        $rows = 0; 
        $rows = mysqli_num_rows($resultBookingPlayerCount); 
       // printf("Number of row in the table : " . $row); 
		if ($rows>0) 
		{
			while($row = mysqli_fetch_array($resultBookingPlayerCount)){
		...
		$resultBookingUserCount = mysqli_query($connection, $queryBookingUserCount); 
		
        $rows = 0; 
        $rows = mysqli_num_rows($resultBookingUserCount); 
		$intBookingUserCount = 0;
       // printf("Number of row in the table : " . $row); 
		if ($rows>0) 
		{
			while($row = mysqli_fetch_array($resultBookingUserCount)){
